...
function Brain.mutateAddConnection(self)
	...
	local inbiased = math.random() < CONNECTION_MUTATION_IN_BIAS
	local outbiased = math.random() < CONNECTION_MUTATION_OUT_BIAS
	local startNodes = {}
	if inbiased then
		startNodes = {length = STATIC_INPUTS,
					[1]=INPUT_NODES - STATIC_INPUTS + 1,
					[2]=INPUT_NODES - STATIC_INPUTS + 2,
					[3]=INPUT_NODES - STATIC_INPUTS + 3,
					[4]=INPUT_NODES - STATIC_INPUTS + 4,
					[5]=INPUT_NODES - STATIC_INPUTS + 5,
					[6]=INPUT_NODES - STATIC_INPUTS + 6,
					[7]=INPUT_NODES - STATIC_INPUTS + 7,
					[8]=INPUT_NODES - STATIC_INPUTS + 8,
					[9]=INPUT_NODES - STATIC_INPUTS + 9,
					[10]=INPUT_NODES - STATIC_INPUTS + 10,
					[11]=INPUT_NODES - STATIC_INPUTS + 11,
					[12]=INPUT_NODES}
	else
		startNodes = self.getAllNodes(self)
	end 
